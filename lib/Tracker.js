...
function Tracker(connection, options) {
    ...
    function processQueue() {
        var current = tracker.queue.getCurrent().message;
        current.trackerId = tracker.trackerId;
        tracker.send(current);
 ...
 * @param {number|string} smsPhone
 * @param {number|string} callPhone
 * @param {function} callback
 */
...
Tracker.prototype.setAlarmMovement = function(area, callback) {
    ...
    if (typeof area !== 'number') {
        callback(new Error('Bad value given, number expected'));
        return;
    }
...
Tracker.prototype.setAlarmGeofence = function(latitude, longitude, radius, callback) {
    ...
    function stuffWithZeroes(value, format){
        ...
        // Fill left part
        while (valueParts[0].length < formatParts[0].length) valueParts[0] = '0' + valueParts[0];
        
        // Fill right part
        if (typeof valueParts[1] === 'undefined') valueParts[1] = '';    
        if (formatParts[1]) {
